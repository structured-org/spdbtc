...
import {ProductParams, BlacklistStorage, WhitelistStorage, WithdrawalRequestsStorage} from './interfaces/IspdBTC.sol';
  ...
  PausableUpgradeable
{
   ...
   */
  bytes32 internal constant _ASSET_SLOT =
    0x5b176b224b19eac6047254b0ea3ab8942dfb79fc40feedc1f9a8173bbe4681fc;
  ...
  bytes32 internal constant _DECIMALS_SLOT =
    0xb444bce2c2faee73cff3f3860f1e7aefa070df3bb7e677ea6c614d5fa351bcbd;

  /**
   * @dev bytes32(uint256(keccak256('spdbtc.max_deposit')) - 1)
   */
  bytes32 internal constant _MAX_DEPOSIT_SLOT =
    0xefc9345aaccbddedbd416aca83be652dad079fbeb16a8b2bae7a6e1558da4b9c;

  /**
   * @dev bytes32(uint256(keccak256('spdbtc.custodian_address')) - 1)
   */
  bytes32 internal constant _CUSTODIAN_SLOT =
    0xcf823567157e2d6a4051cedd374293e4fa81d2713ea60aa14bad53a5a43183f5;

  /**
   * @notice Blacklist functionality like in Tether
   * @dev bytes32(uint256(keccak256('spdbtc.blacklist')) - 1)
   */
  bytes32 internal constant _BLACKLIST_SLOT =
    0x1b78fe6e90a13fd3613f6765435a41cc68067e7d36c75ef0614e997d5fdd5a52;

  /**
   * @notice Whitelist functionality
   * @dev bytes32(uint256(keccak256('spdbtc.whitelist')) - 1)
   */
  bytes32 internal constant _WHITELIST_SLOT =
    0xb6c7ee5a88dde46a513bb443506df08fc209973a5c243f72cf9afea89ed08251;
   ...
   */
  bytes32 internal constant _WITHDRAWAL_REQUESTS_SLOT =
    0x7bf1fc2183ab3f74a444018e145cffd7eb895bd15f852578c1b1954372d8ed46;
  ...
  /**
   * @dev Gets a pointer to the WhitelistStorage struct in storage.
   * @return $ A storage pointer to the WhitelistStorage struct.
   */
  function _getWhitelistStorage()
    internal
    pure
    returns (WhitelistStorage storage $)
  {
    assembly {
      $.slot := _WHITELIST_SLOT
    }
  }
   ...
   */
  function _getWithdrawalRequestsStorage()
    internal
    pure
    returns (WithdrawalRequestsStorage storage $)
  {
    assembly {
  ...
  /// @notice Custom error when the sender of a transaction (deposit) is not whitelisted.
  error SenderIsNotWhitelisted(address sender);
  ...
  /**
   * @notice Emitted when an address is whitelisted or unwhitelisted.
   * @param user The affected address.
  ...
  function _deposit(address caller, address receiver, uint256 amount) internal {
    ...
    _asset.safeTransfer(
      StorageSlot.getAddressSlot(_CUSTODIAN_SLOT).value,
      amount
    );
